...
class WhoisData(BaseModel):
    ...
    raw: StrictStr = Field(description="Raw")
    __properties: ClassVar[List[str]] = ["id", "data", "raw"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in WhoisData) in the input: " + _key)

        _obj = cls.model_validate({
            "id": obj.get("id"),
            "data": WhoisDataObject.from_dict(obj["data"]) if obj.get("data") is not None else None,
            "raw": obj.get("raw")
        })
        return _obj
