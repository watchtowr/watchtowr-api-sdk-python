# coding: utf-8

"""
    Complete watchTowr Platform API Documentation

    The watchTowr Client API combining all watchTowr Platform APIs into a single comprehensive reference, including:       * Continuous Assurance API       * Adversary Sight API       * Intelligence API       * Platform API 

    The version of the OpenAPI document: 1.0
    Contact: support@watchTowr.io
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from watchtowr_api_sdk.models.client_ip_dns_record_pointing_at_data import ClientIpDnsRecordPointingAtData

class TestClientIpDnsRecordPointingAtData(unittest.TestCase):
    """ClientIpDnsRecordPointingAtData unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> ClientIpDnsRecordPointingAtData:
        """Test ClientIpDnsRecordPointingAtData
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `ClientIpDnsRecordPointingAtData`
        """
        model = ClientIpDnsRecordPointingAtData()
        if include_optional:
            return ClientIpDnsRecordPointingAtData(
                data = [
                    watchtowr_api_sdk.models.client_ip_related_dns_record.ClientIpRelatedDnsRecord(
                        id = 1, 
                        name = 'watchtowr.com', 
                        type = 'A', 
                        ttl = 60, 
                        value = '123.123.123.123', 
                        discovered_on = 2022-02-13T02:10:00.000000Z, 
                        asset = watchtowr_api_sdk.models.client_ip_related_dns_record_asset.ClientIpRelatedDnsRecordAsset(
                            id = 1, 
                            name = 'watchtowr.com', ), )
                    ],
                meta = watchtowr_api_sdk.models.meta.Meta(
                    pagination = {"total":20,"count":10,"per_page":10,"current_page":1,"total_pages":2,"links":{"previous":"url","next":"url"}}, )
            )
        else:
            return ClientIpDnsRecordPointingAtData(
                data = [
                    watchtowr_api_sdk.models.client_ip_related_dns_record.ClientIpRelatedDnsRecord(
                        id = 1, 
                        name = 'watchtowr.com', 
                        type = 'A', 
                        ttl = 60, 
                        value = '123.123.123.123', 
                        discovered_on = 2022-02-13T02:10:00.000000Z, 
                        asset = watchtowr_api_sdk.models.client_ip_related_dns_record_asset.ClientIpRelatedDnsRecordAsset(
                            id = 1, 
                            name = 'watchtowr.com', ), )
                    ],
                meta = watchtowr_api_sdk.models.meta.Meta(
                    pagination = {"total":20,"count":10,"per_page":10,"current_page":1,"total_pages":2,"links":{"previous":"url","next":"url"}}, ),
        )
        """

    def testClientIpDnsRecordPointingAtData(self):
        """Test ClientIpDnsRecordPointingAtData"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
